import * as C from "../SearchPage/styled/SearchPage.search.filter.style"
import * as itemS from "../SearchPage/styled/SearchPage.search.filter.period.style"
...
function Period({ onSelectPeriod}) {
    ...
    const selectDayHandle = (e) => {
        ...
        const selectedDate = new Date(fullSelectDay);
        setSelectDay(selectedDate);

        const daysDifference = differenceInCalendarDays(selectedDate, new Date());
        onSelectPeriod({selectedDate, daysDifference});
    }
    ...
    return (
        <>
            <C.FilterContainer type='recruit' onClick={(e) => {
                e.stopPropagation();
            }}>
                <C.FilterContent type='recruit'>
                    <C.FilterTitle>
                        <div>모집 기간</div>
